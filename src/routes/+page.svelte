<script lang="ts">
	...
	let pubCardEl: HTMLElement | null = null;
	let fullyVisible: boolean = false;
	let enablePubScroll: boolean = false;

	function recomputePubScroll() {
		if (!pubCardEl) return;
		const rect = pubCardEl.getBoundingClientRect();
		const hitsTop = rect.top <= 0;
		enablePubScroll = fullyVisible || hitsTop;
	}
	...
	onMount(() => {
		const io = new IntersectionObserver(
			(entries) => {
				for (const e of entries) {
					if (e.target === pubCardEl) {
						fullyVisible = e.intersectionRatio >= 1;
						recomputePubScroll();
					}
				}
			},
		...
		if (pubCardEl) io.observe(pubCardEl);

		const onWinScroll = () => recomputePubScroll();
		const onWinResize = () => recomputePubScroll();
		window.addEventListener('scroll', onWinScroll, { passive: true });
		window.addEventListener('resize', onWinResize);

		setTimeout(recomputePubScroll, 0);
	...
	<div class="max-w-[1000px] w-full bg-white p-2 md:p-4 shadow-sm rounded">
		<div class="flex flex-wrap justify-center mb-2">
			<div class="w-full text-center">
				...
				<p class="text-lg md:text-xl text-gray-700 mb-1">
					...
					<div class="md:basis-[35%] md:max-w-[35%]">
						...
						<div class="flex flex-wrap justify-between gap-2 mt-2">
							...
							<div class="flex items-center gap-2">
								...
								<a href="https://aoli.al"
									><img src={lgbtFlag} class="logo-large" alt="LGBT flag" /></a
								>
							</div>
						...
						<div class="mt-3">
							...
							<div class="mt-3"><a href="{base}/fund">Funding:</a></div>
							<ul class="pl-0">
								<li>
									2025-2026: <a href="https://www.influxdata.com">InfluxData</a>,
									<a href="https://spiraldb.com">Spiral</a>,
									<a href="https://www.bauplanlabs.com">BauPlan</a>
								</li>
								<li>
									2024-2025: <a href="https://www.influxdata.com">InfluxData</a>
								</li>
							</ul>
						</div>
					</div>
